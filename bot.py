...
class WeeklyTradingStrategy:
    ...
    def calculate_fibonacci_levels(self, df, lookback=50):
        ...
        # หาว่าตอนนี้อยู่ใน Uptrend หรือ Downtrend
        current_price = df.iloc[-1]["close"]

        if current_price > (high + low) / 2:
            # Uptrend - คำนวณจาก Low ไป High
            fib_levels = {
                "0.0 (Low)": low,
            ...
            # Downtrend - คำนวณจาก High ไป Low
            fib_levels = {
                "0.0 (High)": high,
    ...
    def get_weekly_signal(self):
        ...
        # 9. ADX (Trend Strength) - ใช้ Daily
        if daily["ADX"] > 25:
            if daily["DI_plus"] > daily["DI_minus"]:
                ...
                reasons["short"].append(
                    f"💪 Strong Downtrend (ADX: {daily['ADX']:.1f})"
                )
        else:
            signals["neutral"] += 2
            reasons["neutral"].append(f"🌊 Weak Trend (ADX: {daily['ADX']:.1f})")

        # 10. Bollinger Bands - ใช้ Daily
    ...
    def calculate_position_management(self, current_price, signal_type):
        """คำนวณการจัดการ Position สำหรับ Weekly"""

        daily_df = self.data["daily"]
        daily = daily_df.iloc[-1]

        # ATR สำหรับ Stop Loss/Take Profit (ใช้ Daily ATR)
        atr_daily = daily["ATR"]

        # Support/Resistance (ใช้ Daily)
        ...
        if signal_type == "LONG":
            # Stop Loss ใช้ Support หรือ 1.5 ATR
            stop_loss_support = sr["main_support"]
            stop_loss_atr = current_price - (atr_daily * 1.5)
            stop_loss = max(stop_loss_support, stop_loss_atr)

            # Take Profit ใช้ Resistance และ Fibonacci
            tp1 = current_price + (atr_daily * 2)  # Conservative
            tp2 = current_price + (atr_daily * 3)  # Moderate
            tp3 = sr["main_resistance"]  # Resistance
            ...
            # Stop Loss ใช้ Resistance หรือ 1.5 ATR
            stop_loss_resistance = sr["main_resistance"]
            stop_loss_atr = current_price + (atr_daily * 1.5)
            stop_loss = min(stop_loss_resistance, stop_loss_atr)

            # Take Profit ใช้ Support และ Fibonacci
            tp1 = current_price - (atr_daily * 2)
            tp2 = current_price - (atr_daily * 3)
            tp3 = sr["main_support"]
    ...
    def get_weekly_recommendation(self, balance=10000):
        ...
        print("\n📅 Weekly Indicators:")
        print(f"  • EMA 9: ${weekly['EMA_9']:,.2f}")
        print(f"  • EMA 21: ${weekly['EMA_21']:,.2f}")
        print(f"  • RSI: {weekly['RSI']:.2f}")
        print(f"  • MACD: {weekly['MACD']:.4f}")

        print("\n📈 Daily Indicators:")
        print(f"  • EMA 9: ${daily['EMA_9']:,.2f}")
        print(f"  • EMA 21: ${daily['EMA_21']:,.2f}")
        print(f"  • RSI: {daily['RSI']:.2f}")
        print(f"  • ADX: {daily['ADX']:.2f}")
        print(f"  • ATR: ${daily['ATR']:,.2f}")

        print("\n⏰ 4H Indicators:")
        print(f"  • EMA 9: ${h4['EMA_9']:,.2f}")
        print(f"  • EMA 21: ${h4['EMA_21']:,.2f}")
        ...
        if signals["long"] > signals["short"] and signals["long"] > signals["neutral"]:
            if long_pct >= 60:
                recommendation = "STRONG_LONG"
            else:
                recommendation = "LONG"
        elif (
            signals["short"] > signals["long"] and signals["short"] > signals["neutral"]
        ):
            if short_pct >= 60:
        ...
        if recommendation in ["STRONG_LONG", "LONG"]:
            ...
            confidence = "STRONG" if recommendation == "STRONG_LONG" else "MODERATE"
            print(f"\n✅ {confidence} LONG SIGNAL ({long_pct:.1f}%)")
            ...
            print(
                f"  🛡️ Stop Loss: ${sl:,.2f} (-{sl_pct:.2f}% = -{sl_pct * self.leverage:.1f}% margin)"
            )
            print(
                f"  🎁 TP1 (40%): ${tp1:,.2f} (+{tp1_pct:.2f}% = +{tp1_pct * self.leverage:.1f}% margin)"
            )
            print(
                f"  🎁 TP2 (30%): ${tp2:,.2f} (+{tp2_pct:.2f}% = +{tp2_pct * self.leverage:.1f}% margin)"
            )
            print(
                f"  🎁 TP3 (30%): ${tp3:,.2f} (+{tp3_pct:.2f}% = +{tp3_pct * self.leverage:.1f}% margin)"
            )

            # Position Size
            risk_pct = 2  # เสี่ยง 2% ต่อ Trade
            risk_amount = balance * (risk_pct / 100)
            position_size = (risk_amount / (sl_pct / 100)) * self.leverage
            ...
            print("\n📊 Risk/Reward Ratio:")
            print(f"  • TP1: 1:{tp1_pct/sl_pct:.2f}")
            print(f"  • TP2: 1:{tp2_pct/sl_pct:.2f}")
            print(f"  • TP3: 1:{tp3_pct/sl_pct:.2f}")
            ...
            for level, price in fib.items():
                marker = (
                    "👉" if abs(price - current_price) / current_price < 0.02 else "  "
                )
                print(f"  {marker} {level}: ${price:,.2f}")

            print("\n📅 WEEKLY STRATEGY:")
            print(f"  1️⃣ เปิด Long ที่ราคาปัจจุบัน ${entry:,.2f}")
            ...
            confidence = "STRONG" if recommendation == "STRONG_SHORT" else "MODERATE"
            print(f"\n❌ {confidence} SHORT SIGNAL ({short_pct:.1f}%)")
            ...
            print("\n📅 WEEKLY STRATEGY:")
            print(f"  1️⃣ เปิด Short ที่ราคาปัจจุบัน ${entry:,.2f}")
            print(f"  2️⃣ ตั้ง Stop Loss ที่ ${sl:,.2f}")
            print("  3️⃣ ปิด 40% ที่ TP1, 30% ที่ TP2, 30% ที่ TP3")
            print("  4️⃣ ถ้าถึง TP1 → ขยับ SL ไปที่ Entry (Break Even)")
            print("  5️⃣ Review ทุกวัน แต่ไม่ต้อง Trade บ่อย")
            ...
            print(f"\n⏸️ WAIT - ไม่มีสัญญาณชัดเจน ({neutral_pct:.1f}%)")
            print("💡 แนะนำ: รอสัญญาณ Weekly ที่ชัดเจนกว่านี้")
            print("📌 ตรวจสอบใหม่อีกครั้งใน 1-2 วัน")

            # แสดง Support/Resistance สำหรับ Monitor
        ...
        print(
            "📅 NEXT REVIEW DATE: "
            + (h4["timestamp"] + timedelta(days=1)).strftime("%Y-%m-%d")
        )
...
class MonthlyTradingStrategy:
    ...
    def get_monthly_signal(self):
        ...
        if (
            monthly_prev["EMA_12"] <= monthly_prev["EMA_26"]
            and monthly["EMA_12"] > monthly["EMA_26"]
        ):
            signals["long"] += 5
        ...
        if (
            weekly_prev["MACD"] <= weekly_prev["MACD_signal"]
            and weekly["MACD"] > weekly["MACD_signal"]
        ):
            signals["long"] += 2
        ...
        if daily["EMA_12"] > daily["EMA_26"]:
            signals["long"] += 1
            reasons["long"].append("📊 Daily Aligned")
        elif daily["EMA_12"] < daily["EMA_26"]:
            signals["short"] += 1
            reasons["short"].append("📊 Daily Aligned")
        ...
        if monthly["ADX"] > 25:
            if monthly["DI_plus"] > monthly["DI_minus"]:
                ...
                signals["short"] += 2
                reasons["short"].append(
                    f"💪 Strong Downtrend (ADX: {monthly['ADX']:.1f})"
                )
        else:
            signals["neutral"] += 2
    ...
    def get_monthly_recommendation(self, balance=10000):
        ...
        print("\n📊 MULTI-TIMEFRAME ANALYSIS:")
        print("\n🌙 Monthly Indicators:")
        print(f"  • EMA 12: ${monthly['EMA_12']:,.2f}")
        print(f"  • EMA 26: ${monthly['EMA_26']:,.2f}")
        print(f"  • RSI: {monthly['RSI']:.2f}")
        print(f"  • MACD: {monthly['MACD']:.4f}")
        print(f"  • ADX: {monthly['ADX']:.2f}")
        print(f"  • ATR: ${monthly['ATR']:,.2f}")

        print("\n📅 Weekly Indicators:")
        print(f"  • EMA 12: ${weekly['EMA_12']:,.2f}")
        print(f"  • EMA 26: ${weekly['EMA_26']:,.2f}")
        print(f"  • RSI: {weekly['RSI']:.2f}")
        print(f"  • MACD: {weekly['MACD']:.4f}")

        print("\n📈 Daily Indicators:")
        print(f"  • EMA 12: ${daily['EMA_12']:,.2f}")
        print(f"  • EMA 26: ${daily['EMA_26']:,.2f}")
        print(f"  • RSI: {daily['RSI']:.2f}")
        ...
        if recommendation in ["STRONG_LONG", "LONG"]:
            ...
            print(
                f"  🎁 TP1 (33%): ${tp1:,.2f} (+{tp1_pct:.2f}% = +{tp1_pct * self.leverage:.1f}% margin)"
            )
            print(
                f"  🎁 TP2 (33%): ${tp2:,.2f} (+{tp2_pct:.2f}% = +{tp2_pct * self.leverage:.1f}% margin)"
            )
            print(
                f"  🎁 TP3 (34%): ${tp3:,.2f} (+{tp3_pct:.2f}% = +{tp3_pct * self.leverage:.1f}% margin)"
            )
            ...
            print("  6️⃣ Hold จนกว่าจะถึง TP หรือ SL หรือสัญญาณ Monthly กลับตัว")
            ...
            print(f"\n⏸️ WAIT - ไม่มีสัญญาณชัดเจน ({neutral_pct:.1f}%)")
            print("💡 แนะนำ: รอสัญญาณ Monthly ที่ชัดเจนกว่านี้")
        ...
        print(
            "📅 NEXT REVIEW DATE: "
            + (daily["timestamp"] + timedelta(days=7)).strftime("%Y-%m-%d")
        )
